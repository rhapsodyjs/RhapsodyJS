...
var Rhapsody = function Rhapsody(options) {
	...
	this.config = this.libs.lodash.merge(this.config, loadConfig(options, Logger));
...
Rhapsody.prototype = {
	...
	generateClientModels: function generateClientModels() {
		...
		if(this.config.clientModels) {
			var clientModelsPath = path.join(this.root, this.config.clientModels.destination || 'app/public/models'),
			...
			var clientModelsPath = path.join(this.root, 'app/public/models'),
		...
		var modelsPath = path.join(this.root, '/app/models');
