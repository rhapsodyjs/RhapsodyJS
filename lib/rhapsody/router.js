  ...
  routeModelsREST: function routeModelsREST(app) {
    ...
    fs.readdirSync(Rhapsody.root + '/models').forEach(function(file) {
      ...
      if((typeof model.allowREST === 'undefined') || model.allowREST) {
        ...
        /* CREATE */
        app.post('/data/' + modelName + '/create', function create(req, res) {
          var dataToCreate = req.query;
          ...
          newData.save(function createData(err) {
            if(err) {
             ...
             responses.respond(res, 201, newData); //Sucessful creation
            }
          });
        });

        /* READ */
        app.get('/data/' + modelName + '/read/:id?', function read(req, res) {
          ...
          if(typeof req.params.id === 'undefined') {
            mongoModel.find({}, function readAllData(err, data) {
              if(err) {
                ...
                responses.respond(res, 200, JSON.stringify(data)); //No error, operation successful
              }
            });
          }
          else {
        ...
        /* UPDATE */
        app.put('/data/' + modelName + '/update/:id', function update(req, res) {
        ...
        /* DELETE */
        app.del('/data/' + modelName + '/delete/:id', function del(req, res) {
