...
var generateModels = function generateModels(rhapsodyApp, buildBackboneModels) {
  ...
  fs.readdirSync(modelsPath).forEach(function(file) {
    if(jsFileRegex.test(file)) {
      ...
      rhapsodyApp.models[modelName.toLowerCase()] = {
  ...
  createRelationships(rhapsodyApp);

};

/**
 * Create all relationships for models
 * @param  {RhapsodyJS app} rhapsodyApp
 */
var createRelationships = function createRelationships(rhapsodyApp) {
  ...
  for(modelName in rhapsodyApp.models) {
    fullModel = rhapsodyApp.requireModel(modelName, true);

    if(fullModel.relationships && Object.keys(fullModel.relationships).length > 0) {
      ...
      for(relationshipName in relationships) {
        ...
        if(fullModel.serverModel[relationship.type]) {
          ...
          if(relatedModel) {
            ...
            if(relationship.type === 'hasAndBelongsToMany') {
              fullModel.serverModel.hasAndBelongsToMany(relationshipName, {
                model: relatedModel
              });
            }
            else {
              fullModel.serverModel[relationship.type](relatedModel, {
                as: relationshipName,
            ...
            rhapsodyApp.log.error('Relationship error', '"' + relationship['with'] + '" related with "' + modelName + '" does not exist.');
          }
        }
        else {
          rhapsodyApp.log.error('Relationship error', relationship.type + ' in "' + modelName + '" is not a valid relationship');
        }
      }
    }
  }
