...
ModelRouter.prototype = {
  ...
  readRelationship: function readRelationship(req, res) {
    var fullModel = req.fullModel;
      ...
      if(err) {
        ...
        if(data[req.params.relationship]) {
          data[req.params.relationship](function readRelationshipData(err, relationshipData) {
            ...
            if(req.query.callback) {
              responseUtils.jsonp(res, 200, relationshipData); //No error, operation successful
            }
            else {
              responseUtils.json(res, 200, relationshipData); //No error, operation successful
            }
          });
        }
        else {
