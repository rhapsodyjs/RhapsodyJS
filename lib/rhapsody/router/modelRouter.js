...
/**
 * Given a model and its data, returns the restrict attributes of the object
 ...
 */
var removeRestrictedAttributes = function removeRestrictedAttributes(data, fullModel) {
  ...
  if(_.isArray(data)) {
    ...
    filteredData = _.omit(data.toObject(), restrictedAttributes);
  }
...
ModelRouter.prototype = {
  ...
  create: function create(req, res) {
    ...
    var dataToCreate = _.omit(req.body, ['_id', '__v']);
    ...
    newData.save(function createData(err) {
      if(err) {
        ...
        var filteredData = removeRestrictedAttributes(newData, fullModel);
  ...
  readMany: function readMany(req, res) {
    ...
    var serverModel = fullModel.serverModel,
        //Splits in the commas to get what attributes the user wants
        requiredAttributes = (req.query.attrs ? req.query.attrs.split(',').join(' ') : undefined);

    serverModel.find({}, requiredAttributes, function readAllData(err, data) {
      if(err) {
        Rhapsody.log.error(err);
        responseUtils.respond(res, 500); //Internal server error
      }
        ...
        //Gets the data without the restricted attributes
        var filteredData = removeRestrictedAttributes(data, fullModel);
  ...
  readOne: function readOne(req, res) {
    ...
    serverModel.findOne({_id: req.params.id}, requiredAttributes, function readData(err, data) {
      if(err) {
        Rhapsody.log.error(err);
        responseUtils.respond(res, 500); //Internal server error
      }
  ...
  update: function update(req, res) {
    ...
    var dataToUpdate = _.omit(req.body, ['_id', '__v']);
    serverModel.update({_id: req.params.id}, dataToUpdate, function updateData(err, affectedCount, aboutUpdate) {
      if(err) {
        if(err.name === 'ValidationError') {
          Rhapsody.log.verbose(err);
          responseUtils.respond(res, 400); //Malformed syntax or a bad query
